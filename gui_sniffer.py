...
def capture_packet(packet):
    ...
    try:
        ...
        packets_list.append((src, dst_name, proto, size))
        tree.insert("", tk.END, values=(src, dst_name, proto, size))
...
def export_to_csv():
    ...
    with open("packets.csv", "w", newline="") as file:
        ...
        writer.writerow(["Source IP", "Destination (Name/IP)", "Protocol", "Size"])
...
protocol_dropdown = ttk.Combobox(root, values=protocol_options,
                                 textvariable=selected_protocol, state="readonly")
...
tree.heading("Destination", text="Destination (Name/IP)")
